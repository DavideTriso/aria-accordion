...
gulp.task('scssDist', function(done) {
  return sass('src/scss/*.scss', {style: 'expanded'}).pipe(autoprefixer('last 5 version')).pipe(gulp.dest('dist')).pipe(rename({suffix: '.min'})).pipe(cssnano()).pipe(gulp.dest('dist')).pipe(notify({message: 'scssDist task complete'}));
  done();
});

//Save a copy of compiled CSS in 'docs' folder
gulp.task('scssDocs', function(done) {
  return sass('src/scss/*.scss', {style: 'expanded'}).pipe(autoprefixer('last 5 version')).pipe(gulp.dest('docs')).pipe(rename({suffix: '.min'})).pipe(notify({message: 'scssDocs task complete'}));
  done();
});
